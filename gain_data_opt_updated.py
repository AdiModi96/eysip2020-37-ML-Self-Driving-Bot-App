...
def make_carla_settings(args):
    ...
    camera0.set_position(2.0, 0.0, 7.4)
...
class CarlaGame(object):
    ...
    def _on_loop(self):
        ...
        loc="G:\\CarlaSimulator\\trimg\\img"
        loc1 = "G:\\CarlaSimulator\\teimg\\img"
        if random.choice(choice)<10:
            imgg.append(loc + str(i) + ".jpg")
            cv2.imwrite(loc + str(i) + ".jpg",img)
            st.append(steer)
            th.append(throttle)
            br.append(brake)
            i = i + 1
        else:
            imggt.append(loc1 + str(j) + ".jpg")
            cv2.imwrite(loc1 + str(j) + ".jpg", img)
            stt.append(steer)
            tht.append(throttle)
            brt.append(brake)
            j = j+1
    ...
    def _get_keyboard_control(self, keys):
        ...
        if button==1:
            ...
            time.sleep(0.0001)
        if axis2 >=0:
            control.throttle=axis2
        if axis3>=0:    # right analog stick when pushed down will apply brakes
            control.brake=axis3
...
if __name__ == '__main__':
    ...
    name_dict['brake'] = br
    name_dictt['Imaget'] = imggt
    name_dictt['Steert'] = stt
    name_dictt['throttlet'] = tht
    name_dictt['braket'] = brt
    df = pd.DataFrame(name_dict)
    dft = pd.DataFrame(name_dictt)
    df.to_csv('Datatr.csv')
    dft.to_csv("Datate.csv")
