...
cv2.createTrackbar("threshmin", "frame", 0, 255,nothing)
cv2.createTrackbar("threshmax", "frame", 0, 255, nothing)
while True:
    img = cv2.imread("G:\\CarlaSimulator\\resize1\\img" + str(0) + ".jpg") 
    img = cv2.cvtColor(img, cv2.COLOR_RGB2GRAY)
    mino = cv2.getTrackbarPos("threshmin", "frame")
    maxo = cv2.getTrackbarPos("threshmax", "frame")
    _ , frame = cv2.threshold(img, mino ,maxo, cv2.THRESH_BINARY)
...
while count <=539:
    img_rgb = cv2.imread("G:\\CarlaSimulator\\resize1\\img" + str(count)+ ".jpg")
    img = cv2.cvtColor(img_rgb, cv2.COLOR_RGB2GRAY)
    _ , img = cv2.threshold(img, mino1 ,maxo1, cv2.THRESH_BINARY)
    ...
    success1,bbox1 = tracker1.update(img_rgb)# for each update the bbox has to be changed this update will give info about whether we r able track the object in the particular frame or not with a bool value
